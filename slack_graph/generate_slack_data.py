...
for channel in filtered_channels:
    store_in_s3(json.dumps(channel), f"channels/{channel['id']}")
...
for channel in filtered_channels:
    ...
    store_in_s3(json.dumps({'members': members}), f"members/{channel['id']}")
...
for user in users:
    store_in_s3(json.dumps(user), f"users/{user['id']}")